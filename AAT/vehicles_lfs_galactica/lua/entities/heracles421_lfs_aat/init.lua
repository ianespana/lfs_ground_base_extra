...
function ENT:Turret( Driver, Pod )
	if not IsValid( Pod ) or not IsValid( Driver ) then
		...
		Driver = Pod:GetDriver()
		if not IsValid( Driver ) or not Driver:lfsGetInput( "FREELOOK" ) then return end
	end
	...
	local TurretPos = self:GetBonePosition( self:LookupBone( "turret_yaw" ) )--self:LocalToWorld( Vector(-130.360611,0,111.885109) )

	local startpos = Pod:GetPos() + EyeAngles:Up() * 100--TurretPos + EyeAngles:Up() * 100
